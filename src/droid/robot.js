...
export function getBaseHeight(baseType) {
  switch (baseType) {
    case 'wheels': return 0.28;
    case 'tracks': return 0.16;
    case 'hovers': return 0.20;
    case 'spider': return 0.25;
    case 'ball': return 0.55;
    case 'legs': return 0.45;
    case 'rotors': return 0.15;
    case 'droideka': return 0.35;
    default: return 0.20;
  }
}
...
function makePBR(scene, color, roughness = 0.45, metallic = 0.5, opts = {}) {
  ...
  if (opts.emissive) {
    mat.emissiveColor = hexToColor3(opts.emissive);
    const intensity = opts.emissiveIntensity !== undefined ? opts.emissiveIntensity : 1.0;
    mat.emissiveColor = mat.emissiveColor.scale(intensity);
  }

  if (opts.emissiveTexture) {
    mat.emissiveTexture = opts.emissiveTexture;
  }

  if (opts.clearCoat) {
    mat.clearCoat = { isEnabled: true, intensity: opts.clearCoat };
  ...
  if (opts.anisotropy) {
    mat.anisotropy = {
      isEnabled: true,
      intensity: opts.anisotropy.intensity !== undefined ? opts.anisotropy.intensity : 0.3,
      axis: opts.anisotropy.axis || new Vector3(1, 0, 0)
    };
  }

  if (opts.subSurface) {
    mat.subSurface = { isEnabled: true, refractionIntensity: opts.subSurface };
    if (opts.subSurfaceColor) {
      const ss = mat.subSurface;
      ss.tintColor = hexToColor3(opts.subSurfaceColor);
    }
  }

  if (opts.transparent) {
    mat.alpha = opts.opacity !== undefined ? opts.opacity : 1.0;
...
function makeStandard(scene, color, opts = {}) {
  // Fall back to improved PBR to prevent cheap look
  return makePBR(scene, color, opts.roughness !== undefined ? opts.roughness : 0.4, opts.metallic !== undefined ? opts.metallic : 0.3, opts);
}
...
function createProceduralPanelTexture(scene, color, opts = {}) {
  ...
  const lineColor = opts.lineColor || 'rgba(0,0,0,0.18)';
  ...
  if (opts.emissiveLines) {
    ctx.strokeStyle = opts.emissiveLines;
    ctx.lineWidth = 1.5;
...
function createProceduralNormalMap(scene, opts = {}) {
  const size = 128;
  const canvas = document.createElement('canvas');
  canvas.width = size;
  canvas.height = size;
  ...
  const gridSize = opts.gridSize || 32;
  ctx.strokeStyle = 'rgb(108, 108, 255)';
  ctx.lineWidth = 2;
  for (let x = 0; x <= size; x += gridSize) {
    ctx.beginPath();
    ctx.moveTo(x, 0);
...
function makePanelPBR(scene, baseColor, roughness = 0.5, metallic = 0.6, panelOpts = {}) {
  ...
  try {
    const tex = createProceduralPanelTexture(scene, '#ffffff', { gridSize: 24, rivets: true, panelLines: true, repeatX: 2, repeatY: 1 });
    mat.baseTexture = tex;
    const normalTex = createProceduralNormalMap(scene, { gridSize: 24, rivets: true, repeatX: 2, repeatY: 1 });
    mat.normalTexture = normalTex;
  } catch (e) { }
  return mat;
}

function makeEmissivePanelPBR(scene, baseColor, emissiveColor, roughness = 0.5, metallic = 0.4, panelOpts = {}) {
  const mat = makePBR(scene, baseColor, roughness, metallic, panelOpts);
  try {
    const tex = createProceduralPanelTexture(scene, '#ffffff', { gridSize: 24, rivets: true, panelLines: true, repeatX: 2, repeatY: 1 });
    mat.baseTexture = tex;
    const normalTex = createProceduralNormalMap(scene, { gridSize: 24, rivets: true, repeatX: 2, repeatY: 1 });
    mat.normalTexture = normalTex;
    const emissiveTex = createProceduralPanelTexture(scene, '#000000', {
      gridSize: 24, rivets: false, panelLines: false, repeatX: 2, repeatY: 1,
      emissiveLines: 'rgba(255,255,255,0.15)',
    });
    mat.emissiveTexture = emissiveTex;
    mat.emissiveColor = hexToColor3(emissiveColor);
  } catch (e) { }
  return mat;
}
...
export function buildDroid(cfg, scene) {
  ...
  const glowMat = makePBR(scene, '#44ddff', 0.1, 0.1, { emissive: '#44ddff', emissiveIntensity: 1.0 });
  const frontGlowMat = makePBR(scene, '#44ffaa', 0.1, 0.1, { emissive: '#22cc88', emissiveIntensity: 1.2 });
  const darkMat = makePBR(scene, '#222222', 0.75, 0.25);
  const chromeMat = makePBR(scene, '#cccccc', 0.15, 0.85, { clearCoat: 0.5 });
  const panelMat = makePanelPBR(scene, accentColor, 0.4, 0.55);
  ...
  const baseType = cfg.base || 'none';
  const baseHeight = getBaseHeight(baseType);
  ...
  switch (cfg.body) {
    case 'sleek': {
      ...
      for (let i = 0; i < 3; i++) {
        const stripe = createBox('stripe', bodyRadius * 1.05, 0.03, 0.04, accentMat, bodyGroup, bodyHeight * (0.2 + i * 0.25));
        stripe.position.z = bodyRadius * 0.7;
      }
    ...
    case 'hover_body': {
      ...
      const hoverGlowMat = makePBR(scene, '#44aaff', 0.1, 0.1, { emissive: '#2266cc', emissiveIntensity: 0.8, transparent: true, opacity: 0.5 });
      ...
      const fanBladeMat = makePBR(scene, '#335566', 0.4, 0.6, { emissive: '#224466', emissiveIntensity: 0.4 });
      for (let i = 0; i < 3; i++) {
        const a = (i * Math.PI * 2) / 3;
        const blade = createBox('fanBlade', bodyRadius * 0.35, 0.015, 0.06, fanBladeMat, bodyGroup, bodyHeight + 0.22, false);
        blade.position.x = Math.cos(a) * bodyRadius * 0.25;
        blade.position.z = Math.sin(a) * bodyRadius * 0.25;
        blade.rotation.y = -a;
      }
      break;
    }
    case 'mech': {
      ...
      const ventMat = makePBR(scene, '#111111', 0.9, 0.1, { emissive: '#331111', emissiveIntensity: 0.3 });
      for (let i = -1; i <= 1; i += 2) {
        for (let j = -1; j <= 1; j += 2) {
          const v = createBox('vent', 0.04, 0.04, 0.06, ventMat, bodyGroup, bodyHeight * 0.5 + j * 0.1);
          v.position.x = i * bodyRadius * 0.4;
          v.position.z = bodyRadius * 0.61;
        }
      }

      const grillMat = makePBR(scene, '#0a0a0a', 0.95, 0.05, { emissive: '#441111', emissiveIntensity: 0.2 });
      for (let i = -2; i <= 2; i++) {
        const grill = createBox('grill', bodyRadius * 1.0, 0.015, 0.02, grillMat, bodyGroup, bodyHeight * 0.3 + i * 0.04);
        grill.position.z = bodyRadius * 0.61;
      }
      break;
    }
    case 'heavy': {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        plate.position.set(Math.cos(a) * (bodyRadius * 1.05), bodyHeight * 0.5, Math.sin(a) * (bodyRadius * 1.05));
        plate.rotation.y = -a + Math.PI / 2;
      }
      const beltMat = makePBR(scene, '#1a1a1a', 0.9, 0.1);
    ...
    case 'slim': {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        pipe.position.x = Math.cos(a) * (bodyRadius * 0.95);
        pipe.position.z = Math.sin(a) * (bodyRadius * 0.95);
      }
    ...
    case 'sphere': {
      ...
      for (let i = -1; i <= 1; i += 2) {
        createSphere('sideLight', 0.05, 8, frontGlowMat, bodyGroup, bodyHeight / 2, false).position.set(i * sr * 0.6, bodyHeight / 2, sr * 0.3);
      }
      const geoMat = makePBR(scene, '#224488', 0.3, 0.1, { emissive: '#112244', emissiveIntensity: 0.4, transparent: true, opacity: 0.3 });
    ...
    case 'insect': {
      ...
      for (let i = -1; i <= 1; i += 2) {
        for (let s = 0; s < 3; s++) {
          ...
          spine.position.set(i * (bodyRadius + 0.12), segH * (s + 0.5) - bodyRadius * 0.1, 0);
          spine.rotation.z = i * Math.PI / 5;
        }
      }
      const wingMat = makePBR(scene, '#4477aa', 0.25, 0.2, { emissive: '#224466', emissiveIntensity: 0.3, transparent: true, opacity: 0.4 });
      for (let side = -1; side <= 1; side += 2) {
        const wingPath = [
          new Vector3(0, 0, 0),
    ...
    case 'tank': {
      ...
      for (const p of armorPlates) {
        createBox('armorPlate', 0.08, bodyHeight * 0.4, bodyRadius * 0.3, panelMat, bodyGroup, p[1]).position.set(p[0], p[1], p[2]);
      }
    ...
    case 'quad': {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        mount.position.x = Math.cos(a) * bodyRadius * 0.9;
        mount.position.z = Math.sin(a) * bodyRadius * 0.9;
      }
      const ringMat = makePBR(scene, '#1a2a3a', 0.8, 0.2, { emissive: '#0a1a2a', emissiveIntensity: 0.3 });
    ...
    case 'astromech_body': {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        p.position.set(Math.cos(a) * abr * 0.92, abh * 0.5, Math.sin(a) * abr * 0.92);
        p.rotation.y = -a + Math.PI / 2;
      }
      for (let j = 0; j < 3; j++) {
        createTorus('astroBelt', abr * 1.9, 0.012, 28, accentMat, bodyGroup, abh * (0.15 + j * 0.35)).rotation.x = Math.PI / 2;
      }
      const ventMat = makePBR(scene, '#0a0a0a', 0.95, 0.05, { emissive: '#220000', emissiveIntensity: 0.2 });
      for (let i = 0; i < 2; i++) {
        for (let j = 0; j < 4; j++) {
      ...
      for (let side = -1; side <= 1; side += 2) {
        createBox('utilityArm', 0.04, 0.03, 0.08, utilityMat, bodyGroup, abh * 0.35, false).position.set(side * abr * 0.9, abh * 0.35, abr * 0.5);
      }
    ...
    case 'protocol_body': {
      ...
      const indicatorMat = makePBR(scene, '#44ff44', 0.1, 0.1, { emissive: '#22cc22', emissiveIntensity: 0.9 });
      for (let i = 0; i < 3; i++) {
        createSphere('protoLight', 0.025, 8, indicatorMat, bodyGroup, pbh * 0.55 - i * 0.05, false).position.set(i * 0.06 - 0.06, pbh * 0.55 - i * 0.05, pbd / 2 + 0.03);
      }
      const wireMat = makePBR(scene, '#554422', 0.6, 0.3);
      for (let side = -1; side <= 1; side += 2) {
        const wirePath = [
    ...
    case 'assassin': {
      ...
      for (let side = -1; side <= 1; side += 2) {
        for (let p = 0; p < 3; p++) {
          ...
          piston.position.x = side * arb * 0.6;
          piston.rotation.z = side * Math.PI / 3;
        }
      }
      const eyeMat = makePBR(scene, '#ff1100', 0.1, 0.1, { emissive: '#ff1100', emissiveIntensity: 2.0 });
      createSphere('assEye', 0.04, 8, eyeMat, bodyGroup, arh * 0.8, false).position.set(0, arh * 0.8, arb * 0.5);
      break;
    }
    default: {
      ...
      const coreMat = makePBR(scene, '#33aaff', 0.1, 0.1, { emissive: '#1166dd', emissiveIntensity: 2.5 });
      ...
      for (let i = 0; i < 3; i++) {
        ...
        strut.position.set(Math.cos(a) * (bodyRadius - 0.02), bodyHeight / 2, Math.sin(a) * (bodyRadius - 0.02));
      }
      break;
    }
  }
  ...
  createBox('rearPanel', bodyRadius * 0.4, bodyHeight * 0.06, 0.03, makePBR(scene, '#cc2233', 0.1, 0.1, { emissive: '#881122', emissiveIntensity: 0.8 }), bodyGroup, bodyHeight * 0.4).position.z = -bodyRadius * 0.98;
  ...
  const headY = baseHeight + bodyHeight + 0.05;
  ...
  switch (cfg.head) {
    case 'dome': {
      ...
      createBox('visor', bodyRadius * 0.5, 0.03, 0.08, makePBR(scene, '#111122', 0.1, 0.1), headGroup, headY + 0.08).position.z = bodyRadius * 0.6;
      createSphere('rearDot', 0.05, 8, makePBR(scene, '#ff3344', 0.1, 0.1, { emissive: '#cc1122', emissiveIntensity: 0.8 }), headGroup, headY + 0.1, false).position.z = -bodyRadius * 0.6;
    ...
    case 'sensor': {
      ...
      for (let i = -1; i <= 1; i += 2) {
        ...
        createTorus('rim', 0.11, 0.008, 14, chromeMat, headGroup, headY + 0.11).position.set(i * 0.18, headY + 0.11, sd / 2 + 0.045);
      }
      ...
      rearLens.material = makePBR(scene, '#ff3344', 0.1, 0.1, { emissive: '#cc1122', emissiveIntensity: 0.8 });
    ...
    case 'visor': {
      ...
      createBox('visorSlit', bodyRadius * 1.2, 0.06, 0.05, makePBR(scene, '#111122', 0.15, 0.1), headGroup, headY + 0.09).position.z = bodyRadius * 0.31;
      ...
      createSphere('rearDot', 0.04, 8, makePBR(scene, '#ff3344', 0.1, 0.1, { emissive: '#cc1122', emissiveIntensity: 0.8 }), headGroup, headY + 0.09, false).position.z = -bodyRadius * 0.31;
    ...
    case 'box': {
      ...
      for (let i = -1; i <= 1; i += 2) {
        createSphere('eye', 0.08, 10, frontGlowMat, headGroup, headY + 0.18, false).position.set(i * 0.12, headY + 0.18, bodyRadius * 0.42);
        createTorus('eyeRim', 0.09, 0.008, 12, chromeMat, headGroup, headY + 0.18).position.set(i * 0.12, headY + 0.18, bodyRadius * 0.43);
      }
      createBox('mouth', bodyRadius * 0.4, 0.025, 0.03, makePBR(scene, '#222222', 0.7, 0.3, { emissive: '#331111', emissiveIntensity: 0.4 }), headGroup, headY + 0.08).position.z = bodyRadius * 0.42;
      for (let i = -1; i <= 1; i += 2) {
      ...
      createSphere('rearLight', 0.04, 8, makePBR(scene, '#ff3344', 0.1, 0.1, { emissive: '#cc1122', emissiveIntensity: 0.8 }), headGroup, headY + 0.15, false).position.z = -bodyRadius * 0.41;

      const cheekMat = makePBR(scene, '#2244aa', 0.2, 0.8, { emissive: '#112266', emissiveIntensity: 0.5 });
      for (let side = -1; side <= 1; side += 2) {
        createBox('cheek', 0.06, 0.08, 0.04, cheekMat, headGroup, headY + 0.12).position.set(side * bodyRadius * 0.5, headY + 0.12, bodyRadius * 0.35);
      }
      break;
    }
    case 'turret': {
    ...
    case 'cyclops': {
      ...
      const eyeMat = makePBR(scene, '#ff2200', 0.1, 0.1, { emissive: '#ff4400', emissiveIntensity: 2.2 });
      ...
      for (let i = 0; i < 4; i++) {
        ...
        plate.position.set(Math.cos(a) * bodyRadius * 0.48, headY + 0.1 + Math.sin(a) * 0.02, bodyRadius * 0.12);
      }
      for (let i = -1; i <= 1; i += 2) {
        createSphere('sideEye', 0.06, 10, frontGlowMat, headGroup, headY + 0.1, false).position.set(i * bodyRadius * 0.4, headY + 0.1, bodyRadius * 0.0);
      }
      break;
    }
    case 'crown': {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        spike.position.set(Math.cos(a) * bodyRadius * 0.42, headY + 0.28, Math.sin(a) * bodyRadius * 0.42);
        createSphere('spikeTip', 0.03, 8, glowMat, headGroup, headY + 0.48, false).position.set(Math.cos(a) * bodyRadius * 0.42, headY + 0.48, Math.sin(a) * bodyRadius * 0.42);
      }
    ...
    case 'tri_eye': {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const eyeMat = makePBR(scene, eyeColor, 0.1, 0.1, { emissive: eyeColor, emissiveIntensity: 1.5 });
    ...
    case 'goggle': {
      ...
      const lensMat = makePBR(scene, '#44aaff', 0.1, 0.1, { emissive: '#2266cc', emissiveIntensity: 0.6, transparent: true, opacity: 0.7 });
      for (let side = -1; side <= 1; side += 2) {
        ...
        lens.position.set(side * 0.14, headY + 0.08, bodyRadius * 0.36);
        lens.scaling.z = 0.5;
      }
      createBox('goggleBridge', 0.06, 0.04, 0.04, chromeMat, headGroup, headY + 0.08).position.z = bodyRadius * 0.36;
      createSphere('goggleRear', 0.04, 8, makePBR(scene, '#ff3344', 0.1, 0.1, { emissive: '#cc1122', emissiveIntensity: 0.8 }), headGroup, headY + 0.06, false).position.z = -bodyRadius * 0.36;
      break;
    }
    case 'halo': {
      createSphere('haloHead', bodyRadius * 0.5 * 2, 24, headMat, headGroup, headY + 0.04);
      const haloMat = makePBR(scene, '#4488ff', 0.1, 0.1, { emissive: '#4488ff', emissiveIntensity: 1.0, transparent: true, opacity: 0.7 });
      const haloRing = createTorus('haloRing', bodyRadius * 1.0 * 2, 0.015, 24, haloMat, headGroup, headY + 0.2);
      haloRing.rotation.x = Math.PI / 2;
      const haloGlow = makePBR(scene, '#4488ff', 0.1, 0.1, { emissive: '#4488ff', emissiveIntensity: 0.4, transparent: true, opacity: 0.3 });
    ...
    case 'astromech': {
      ...
      const bluePanel = makePBR(scene, '#2255bb', 0.4, 0.5, { emissive: '#1133aa', emissiveIntensity: 0.6 });
      createBox('astroBluePanel', bodyRadius * 0.3, 0.04, 0.08, bluePanel, headGroup, headY + 0.08).position.set(-bodyRadius * 0.2, headY + 0.08, bodyRadius * 0.7);
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const lMat = makePBR(scene, sensorColors[i], 0.1, 0.1, { emissive: sensorColors[i], emissiveIntensity: 0.8 });
        createSphere('astroSensor_' + i, 0.035, 8, lMat, headGroup, headY + 0.05, false).position.set(sx, headY + 0.05, sz);
      }
    ...
    case 'protocol': {
      ...
      head.scaling.y = 0.8;
      head.scaling.z = 0.75;
      const protoEyeMat = makePBR(scene, '#ffcc44', 0.1, 0.1, { emissive: '#ffcc44', emissiveIntensity: 1.2 });
      for (let side = -1; side <= 1; side += 2) {
        ...
        eye.position.set(side * 0.09, headY + 0.06, bodyRadius * 0.55);
        eye.rotation.x = Math.PI / 2;
        createTorus('protoEyeRim_' + side, 0.13, 0.012, 16, darkMat, headGroup, headY + 0.06).position.set(side * 0.09, headY + 0.06, bodyRadius * 0.56);
      }
      const mouthMat = makePBR(scene, '#332200', 0.7, 0.3, { emissive: '#110000', emissiveIntensity: 0.2 });
      for (let i = -1; i <= 1; i++) {
        createBox('protoMouth_' + i, 0.08, 0.012, 0.01, mouthMat, headGroup, headY - 0.02).position.set(i * 0.08, headY - 0.02, bodyRadius * 0.55);
      }
      for (let side = -1; side <= 1; side += 2) {
        const ear = createCylinder('protoEar_' + side, 0.04, 0.06, 0.1, 12, protoGoldMat, headGroup, headY + 0.02, false);
        ear.position.set(side * bodyRadius * 0.65, headY + 0.02, 0);
        ear.rotation.z = Math.PI / 2;
        createTorus('protoEarRing_' + side, 0.1, 0.01, 12, accentMat, headGroup, headY + 0.02).position.set(side * bodyRadius * 0.68, headY + 0.02, 0);
      }
      break;
    }
  }
  ...
  switch (cfg.base) {
    case 'wheels': {
      ...
      for (const p of positions) {
        ...
        w.position.set(p[0], 0.06, p[2]);
        w.rotation.z = Math.PI / 2;
        wheelsList.push(w);
        createCylinder('hub', 0.03, 0.03, 0.13, 8, chromeMat, baseGroup, 0.06, false).position.set(p[0], 0.06, p[2]);
        ...
        for (let s = 0; s < 5; s++) {
          ...
          spoke.position.set(p[0] + Math.sin(sa) * 0.04, 0.06, p[2] + Math.cos(sa) * 0.04);
          spoke.rotation.y = sa;
        }
      }
      root.metadata = root.metadata || {};
      ...
      const legHeight = baseHeight - 0.06;
      for (const p of positions) {
        createCylinder('leg', 0.035, 0.055, legHeight, 10, legMat, baseGroup, (baseHeight + 0.06) / 2, false).position.set(p[0], (baseHeight + 0.06) / 2, p[2]);
      }
      break;
    }
    case 'tracks': {
      ...
      for (let side = -1; side <= 1; side += 2) {
        ...
        const rollerMat = makePBR(scene, '#111111', 0.8, 0.2);
        for (let i = -2; i <= 2; i++) {
        ...
        const beltMat = makePBR(scene, '#1a1a1a', 0.9, 0.1);
        for (let i = -2; i <= 2; i++) {
          createBox('belt', trackW + 0.02, 0.025, 0.04, beltMat, baseGroup, trackH - 0.01, false).position.set(side * (bodyRadius + 0.07), trackH - 0.01, i * trackD / 5);
        }
        const treadMat = makePBR(scene, '#2a2a2a', 0.8, 0.15);
        for (let i = -5; i <= 5; i++) {
          createBox('tread', trackW + 0.03, 0.03, 0.015, treadMat, baseGroup, trackH * 0.85, false).position.set(side * (bodyRadius + 0.07), trackH * 0.85, i * trackD / 11);
        }
      }
      
      // Central chassis to connect tracks
      createBox('chassis', bodyRadius * 1.4, 0.08, trackD * 0.8, darkMat, baseGroup, 0.12);
    ...
    case 'hovers': {
      ...
      const glowMatH = makePBR(scene, '#44aaff', 0.1, 0.1, { emissive: '#2266cc', emissiveIntensity: 0.8, transparent: true, opacity: 0.8 });
      const positions = [
        [0, 0, bodyRadius * 1.0],
      ...
      const hoverGlows = [];
      for (const p of positions) {
        createCylinder('pad', 0.13, 0.16, 0.06, 16, hoverMat, baseGroup, 0.04, false).position.set(p[0], 0.04, p[2]);
        ...
        createTorus('padRing', 0.28, 0.008, 16, chromeMat, baseGroup, 0.05, false).position.set(p[0], 0.05, p[2]);
        
        // Strut connector from each pad to the body
        createCylinder('padStrut', 0.025, 0.025, baseHeight - 0.07, 8, darkMat, baseGroup, (baseHeight + 0.07) / 2, false).position.set(p[0], (baseHeight + 0.07) / 2, p[2]);
      }
      root.metadata = root.metadata || {};
    ...
    case 'spider': {
      const mountHeight = baseHeight;
      createCylinder('spiderMount', bodyRadius * 0.35, bodyRadius * 0.5, mountHeight, 16, baseMat, baseGroup, mountHeight / 2);
      const legMat = makePBR(scene, baseColor, 0.6, 0.5);
      for (let i = 0; i < 4; i++) {
        const a = (i * Math.PI) / 2 + Math.PI / 4;
        const legG = new TransformNode('leg_' + i, scene);
        legG.parent = baseGroup;
        legG.position.set(Math.cos(a) * bodyRadius * 0.3, baseHeight - 0.04, Math.sin(a) * bodyRadius * 0.3);
        legG.rotation.y = a;
        
        createCylinder('upJoint', 0.04, 0.05, 0.05, 10, darkMat, legG, 0);
        ...
        addMesh(upLeg, legG, 0.06);
        upLeg.position.z = 0.08;
        upLeg.rotation.x = -Math.PI / 6; // tilt up-outward
        
        const loLeg = MeshBuilder.CreateCylinder('loLeg', { diameterTop: 0.03, diameterBottom: 0.05, height: 0.38, tessellation: 10 }, scene);
        loLeg.material = legMat;
        addMesh(loLeg, legG, -0.08);
        loLeg.position.z = 0.28;
        loLeg.rotation.x = Math.PI / 4; // tilt down-outward
        ...
        addMesh(foot, legG, -baseHeight + 0.04);
        foot.position.z = 0.42;
        
        createSphere('kneeJoint', 0.045, 8, chromeMat, legG, 0.12).position.z = 0.18;
      }
      break;
    }
    case 'ball': {
      const ballR = 0.22;
      const ball = MeshBuilder.CreateSphere('ballBase', { diameter: ballR * 2, segments: 32 }, scene);
      ball.material = baseMat;
      addMesh(ball, baseGroup, ballR);
      
      createTorus('ballEquator', ballR * 2.05, 0.025, 32, accentMat, baseGroup, ballR).rotation.x = Math.PI / 2;
      const treadMat = makePBR(scene, '#111111', 0.95, 0.05);
      for (let i = -1; i <= 1; i += 2) {
        createTorus('tread', ballR * 2.08, 0.015, 28, treadMat, baseGroup, ballR + i * ballR * 0.5);
      }
      
      // Connector/socket from the ball to the body
      createCylinder('ballConn', bodyRadius * 0.35, bodyRadius * 0.4, baseHeight - ballR * 2 + 0.02, 16, accentMat, baseGroup, (baseHeight + ballR * 2) / 2);
      break;
    }
    case 'legs': {
      const hipMat = makePBR(scene, baseColor, 0.5, 0.6);
      createCylinder('hipMount', bodyRadius * 0.3, bodyRadius * 0.4, 0.06, 16, hipMat, baseGroup, baseHeight - 0.03);
      for (let i = 0; i < 2; i++) {
        const side = i === 0 ? -1 : 1;
        const legRoot = new TransformNode('bipedLeg_' + i, scene);
        legRoot.parent = baseGroup;
        legRoot.position.set(side * bodyRadius * 0.3, baseHeight - 0.06, 0);

        const thighMat = makePBR(scene, baseColor, 0.6, 0.5);
        const thigh = MeshBuilder.CreateCylinder('thigh', { diameterTop: 0.05, diameterBottom: 0.07, height: 0.18, tessellation: 10 }, scene);
        thigh.material = thighMat;
        addMesh(thigh, legRoot, -0.08);
        thigh.rotation.x = -0.15;
        thigh.position.z = 0.04;

        const shinMat = makePBR(scene, baseColor, 0.65, 0.4);
        const shin = MeshBuilder.CreateCylinder('shin', { diameterTop: 0.04, diameterBottom: 0.06, height: 0.22, tessellation: 10 }, scene);
        shin.material = shinMat;
        addMesh(shin, legRoot, -0.26);
        shin.rotation.x = 0.2;
        shin.position.z = 0.08;

        createSphere('knee', 0.05, 8, chromeMat, legRoot, -0.16).position.z = 0.06;
        createBox('foot', 0.08, 0.025, 0.12, darkMat, legRoot, -0.38).position.z = 0.1;
      }
      break;
    }
    case 'rotors': {
      ...
      for (let ring = 0; ring < 2; ring++) {
        for (let i = 0; i < 4; i++) {
          ...
          blade.position.set(Math.cos(a) * 0.25, 0.05 + ring * 0.04, Math.sin(a) * 0.25);
          blade.rotation.y = -a;
          rotorBlades.push(blade);
        }
      }
      // Add a connector shaft
      createCylinder('rotorShaft', 0.03, 0.03, baseHeight - 0.09, 8, darkMat, baseGroup, (baseHeight + 0.09) / 2);
      root.metadata = root.metadata || {};
    ...
    case 'droideka': {
      ...
      const shieldMat = makePBR(scene, '#664422', 0.2, 0.1, { emissive: '#332200', emissiveIntensity: 0.3, transparent: true, opacity: 0.55 });
      for (let i = 0; i < 6; i++) {
        const a = (i * Math.PI * 2) / 6;
        ...
        shield.position.set(Math.cos(a) * dkr * 0.6, dkr * 0.7 + Math.sin(a) * 0.15, Math.sin(a) * dkr * 0.5);
        shield.rotation.z = -a;
        shield.rotation.y = a + Math.PI / 2;
      }
      const dkGlowMat = makePBR(scene, '#ff4400', 0.1, 0.1, { emissive: '#ff4400', emissiveIntensity: 1.0, transparent: true, opacity: 0.8 });
      ...
      for (let side = -1; side <= 1; side += 2) {
        ...
        blaster.position.set(side * dkr * 0.6, dkr * 0.5, dkr * 0.3);
        blaster.rotation.x = Math.PI / 2;
        blaster.rotation.z = side * Math.PI / 5;
        createTorus('dkBlasterRing_' + side, 0.06, 0.008, 10, accentMat, baseGroup, dkr * 0.5).position.set(side * dkr * 0.65, dkr * 0.5, dkr * 0.48);
      }
      const dkShields = [];
  ...
  switch (cfg.accessory) {
    case 'arm': {
      createSphere('shoulder', 0.12, 12, chromeMat, bodyGroup, 0, false).position.set(bodyRadius * 0.85, bodyHeight * 0.55, 0);
      createCylinder('arm', 0.025, 0.03, 0.3, 10, accMat, bodyGroup, 0, false).position.set(bodyRadius * 0.85 + 0.15, bodyHeight * 0.45, 0);
      bodyGroup.getChildren()[bodyGroup.getChildren().length - 1].rotation.z = Math.PI / 4;
      createSphere('elbow', 0.07, 10, chromeMat, bodyGroup, 0, false).position.set(bodyRadius * 0.85 + 0.26, bodyHeight * 0.35, 0);
      createCylinder('forearm', 0.02, 0.025, 0.2, 10, accMat, bodyGroup, 0, false).position.set(bodyRadius * 0.85 + 0.26, bodyHeight * 0.25, 0);
      createBox('claw', 0.06, 0.015, 0.04, chromeMat, bodyGroup, 0, false).position.set(bodyRadius * 0.85 + 0.26, bodyHeight * 0.15, 0);
      break;
    }
    case 'shield': {
      const orb = MeshBuilder.CreateIcoSphere('shieldOrb', { radius: 0.12, subdivisions: 2 }, scene);
      orb.material = makePBR(scene, '#44aaff', 0.1, 0.1, { emissive: '#2266cc', emissiveIntensity: 0.8, transparent: true, opacity: 0.8 });
      addMesh(orb, bodyGroup, bodyHeight * 0.5, false);
      orb.position.set(0, bodyHeight * 0.5, -bodyRadius * 0.85);
      createCylinder('mount', 0.03, 0.04, 0.15, 10, darkMat, bodyGroup, bodyHeight * 0.5, false).position.set(0, bodyHeight * 0.5, -bodyRadius * 0.7);
      bodyGroup.getChildren()[bodyGroup.getChildren().length - 1].rotation.x = Math.PI / 2;
      const shieldMat = makePBR(scene, '#88ccff', 0.1, 0.1, { emissive: '#4488cc', emissiveIntensity: 0.5, transparent: true, opacity: 0.55 });
      createTorus('shieldRing', 0.36, 0.012, 24, shieldMat, bodyGroup, bodyHeight * 0.5, false).position.set(0, bodyHeight * 0.5, -bodyRadius * 0.85);
      break;
    }
    case 'jetpack': {
      const jetpackFlames = [];
      for (let side = -1; side <= 1; side += 2) {
        createCylinder('pod', 0.06, 0.08, 0.3, 12, accMat, bodyGroup, bodyHeight * 0.5).position.set(side * (bodyRadius * 0.7), bodyHeight * 0.5, -bodyRadius * 0.3);
        createCylinder('nozzle', 0.04, 0.06, 0.08, 12, chromeMat, bodyGroup, bodyHeight * 0.32).position.set(side * (bodyRadius * 0.7), bodyHeight * 0.32, -bodyRadius * 0.3);
        const flameMat = makePBR(scene, '#44aaff', 0.1, 0.1, { emissive: '#2266ff', emissiveIntensity: 1.5, transparent: true, opacity: 0.85 });
        const flame = MeshBuilder.CreateCylinder('jpFlame', { diameterTop: 0, diameterBottom: 0.1, height: 0.15, tessellation: 10 }, scene);
        flame.material = flameMat;
        addMesh(flame, bodyGroup, bodyHeight * 0.22, false);
        flame.position.set(side * (bodyRadius * 0.7), bodyHeight * 0.22, -bodyRadius * 0.3);
        flame.rotation.x = Math.PI;
        jetpackFlames.push(flame);
      }
      createBox('strap', bodyRadius * 1.4, 0.03, 0.03, darkMat, bodyGroup, bodyHeight * 0.5, false).position.set(0, bodyHeight * 0.5, -bodyRadius * 0.3);
      root.metadata = root.metadata || {};
      root.metadata.jetpackFlames = jetpackFlames;
      break;
    }
    case 'radar': {
      createCylinder('radarArm', 0.02, 0.025, 0.22, 8, darkMat, bodyGroup, bodyHeight + 0.11, false).position.set(bodyRadius * 0.5, bodyHeight + 0.11, 0);
      const radarHead = new TransformNode('radarHead', scene);
      radarHead.parent = bodyGroup;
    ...
    case 'drone': {
      ...
      const droneWingMat = makePBR(scene, '#335577', 0.2, 0.1, { emissive: '#224466', emissiveIntensity: 0.4, transparent: true, opacity: 0.5 });
      for (let i = 0; i < 3; i++) {
        const a = (i * Math.PI * 2) / 3;
    ...
    case 'wing': {
      ...
      const wingTransMat = makePBR(scene, '#446688', 0.2, 0.1, { emissive: '#224466', emissiveIntensity: 0.5, transparent: true, opacity: 0.6 });
      for (let side = -1; side <= 1; side += 2) {
        const wingG = new TransformNode('wing_' + side, scene);
        wingG.parent = bodyGroup;
  ...
  // Forward indicator
  const fwdMat = makePBR(scene, '#33ffaa', 0.1, 0.1, { emissive: '#00ff88', emissiveIntensity: 2.2, transparent: true, opacity: 0.85 });
